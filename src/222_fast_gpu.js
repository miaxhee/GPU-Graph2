...
var graflength = 400 //1500               //hier muss man die Anzahl Knoten einstellen   und stelle weiter unten auch die glenth bei OPT2 richtig ein
...
//     graf = myGPUfuncReturn(ret_grafXY, graf);  //die kernel fkt merged die forces aus ret_grafXY zurück ins graf array

	 
	 graf = superKernel(graf, has_edge);
...
*/ 
  ...
  step();
	...
	const opt2 = {
		constants: { glength: 364 },    // muss eingestellt werden mit wert graflength
...
//diese Kernel Funktion merged die Kräfte aus forceXY ins graf array
    ...
    const myGPUfuncReturn = gpu.createKernel(function(ret_grafXY, graf){
	...
	const superKernel = gpu.combineKernels(myGPUfuncXY, myGPUfuncReturn, function(graf, has_edge) {
		return myGPUfuncReturn(myGPUfuncXY(graf, has_edge), graf);
	});
